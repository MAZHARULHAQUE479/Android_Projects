...
import com.tamtoanthang.apps.mobileparking.CameraPreview;
...
//        sQLiteHelper = new SQLiteHelper( getActivity());
        ...
        System.out.println("=======BaseUrl123=========="+BaseUrl);
        ...
        bt1.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                In_time = dateFormat.format(new Date());
...
//                Baseimage = Base64.encodeToString(imageBytes, Base64.DEFAULT);
                prefstatus=mListener.getBaseContext().getSharedPreferences("Mystatus",Context.MODE_PRIVATE);
                SessionStatus=prefstatus.getString("message","");
                System.out.println("========SessionStatus========="+SessionStatus);
                im=getActivity().getSharedPreferences("img",Context.MODE_PRIVATE);
                Baseimage=im.getString("im","");
                System.out.println("========Baseimage1========="+Baseimage);
                ...
                if (SessionStatus.equals("")) {
                    ...
                    {
                        RegisterUpdate(CardNo,In_time,Baseimage);

                    }


//                dhg2 = ed1.getText().toString();
//
//                Log.d("mrk", "vmkm" + dhg2);


//               String mkvm=  getContact1(dhg2, SecondActivity.price);
//
//                if ((dhg2 != null) ) {
//                    dbHelper.getWritableDatabase();
//                    sQLiteHelper.getWritableDatabase();
//                    Log.d("&&&","D2Fragment");
//                    ArrayList<Contact> contact = dbHelper.getContactwithcrdno(dhg2);
//                    ArrayList<ContactModel> contact1 = sQLiteHelper.getAllRecords1(ed1.getText().toString(),ed1.getText().toString());
//
//                    Log.d("card_id",""+dhg2+contact.size());
//
//                        if(containsCardno(contact1,ed1.getText().toString())||(containscardid(contact1,ed1.getText().toString()))){
//                            if (!mkvm.equals("")) {
//
//                                if (contact.size() == 0) {
//
//
//                                    Insert();
//                                    getContactimage(dhg2, galleryimage);
//                                } else {
//                                    Log.d("size", "" + contact.size());
//                                    boolean isNotUpdated = true;
//                                    ArrayList<Contact> contact11 = dbHelper.getContact1(dhg2,"1");
//                                    for (int a = 0; a < contact11.size(); a++) {
//
//                                        if ((contact11.get(a).getCardNo().equals(dhg2)) && (contact11.get(a).getStatus().equalsIgnoreCase("1"))) {
//
//
//                                            update();
//
...
//                                }
//                            }else {
//                                Toast.makeText(getActivity(), getString(R.string.Please_enter_cardprice_in_admin_panel), Toast.LENGTH_SHORT).show();
//                            }
//                        }else {
//                            Toast.makeText(getActivity(), getString(R.string.Please_enter_valid_card_no), Toast.LENGTH_SHORT).show();
//                        }
//                    dhg2="";
//                    cardprice="";
//
...
//             Log.d("editTecte","cj"+editText+contacts.get(r).getCardId());
...
//        }
//
//            ArrayList<ContactModel> contacts2 = sQLiteHelper.getAllRecordsPrice();
...
//
//                }Log.d("cardprice","fjnei"+cardprice);
//
//            }
//           return cardprice;
//
...
//
//        ArrayList<Contact> contacts =dbHelper. getContactwithcrdno(jndj);
//        int Id = 0;
//
...
//            if (jndj.equals(contacts.get(k).getCardNo())) {
//                try {
//                    Bitmap b = BitmapFactory.decodeStream(new FileInputStream(filenameInsert));
...
//
//    }
   /* public void getContactimage1(int pos) {
        dbHelper = new DatabaseHelper(getActivity());

         byte[]img=SecondActivity.dataimage;
        ArrayList<Contact> contact11 = dbHelper.getContact1(dhg2,"1");

        Log.d("jwjifjiwfj",""+contact11.size());

        for (int k = 0; k < contact11.size(); k++) {
            Log.d("jwjifjiwfj",""+contact11.get(k).getCardNo()+"%%"+cardNo+"##"+contact11.get(k).getCardIdNo());
            SimpleDateFormat dateFormat = new SimpleDateFormat("yyyymmddhhmmss");
            String date = dateFormat.format(new Date());


                if ((contact11.get(k).getCardNo().equals(dhg2)) && (contact11.get(k).getStatus().equalsIgnoreCase("1"))) {
                    //  String t =contact.get(i).getInTime();
                    position = k;
                    Log.d("jwjifjiwfj",""+position+"##");
                }


           // if (jndj.equals(contacts.get(k).getCardNo())) {
            byte[]cm=contact11.get(position).getInImage();
        ...
        class SendPostReqAsyncTask extends AsyncTask<String, Void, String> {
            ...
            protected String doInBackground(String... params) {
                ...
                try {
                    ...
                    HttpPost httpPost = new HttpPost(BaseUrl+"/transaction_create_manual.php");
...
//                        String sucesss=jsonObject.getString("messsage1");
//                        System.out.println("===sucesss===="+sucesss);
                        Toast.makeText(mListener.getBaseContext(), "Please Come In", Toast.LENGTH_SHORT).show();
                        ...
                        if (message!= null) {
                            ...
                            for (int i = 0; i < message.length(); i++) {
                                ...
                                String eid = m.getString("id");
                                String nn = m.getString("user_name");
                                String cid = m.getString("card_id");
                                mListener.id.setText(cid);
                                String cno = m.getString("card_no");
                                String ct = m.getString("card_type");
                                String cp = m.getString("card_price");
                                CardPrice = m.getString("card_price");
                                mListener. price.setText(CardPrice);
                                InImage = m.getString("in_image");
                                Picasso.with(mListener).load(img+InImage).into(galleryimage);
                                im = mListener.getBaseContext().getSharedPreferences("img", MODE_PRIVATE);
                                SharedPreferences.Editor editor1 = im.edit();
                                editor1.clear();
                                editor1.commit();
...
//                                        .into(galleryimage);
                                String OutImage = m.getString("out_image");
                                ...
                                String intime = m.getString("in_time");
                                String outtime = m.getString("out_time");
                                String login = m.getString("login_time");
//                                System.out.println("===message===="+sucesss);
                        ...
                        String  errormessage = jsonObject.getString("messsage");
                        if (errormessage!= null) {
                            {
                                System.out.println("===========================error");
                                Toast.makeText(mListener.getBaseContext(), errormessage, Toast.LENGTH_SHORT).show();
                            }
                        }
                    }
                } catch (JSONException e) {
    ...
    public void  RegisterUpdate(
        ...
        class SendPostReqAsyncTask extends AsyncTask<String, Void, String> {
            ...
            protected String doInBackground(String... params) {
                ...
                try {
                    HttpClient httpClient = new DefaultHttpClient();
                    HttpPost httpPost = new HttpPost(BaseUrl+"/transaction_update_manual.php");
                    StringEntity httpiEntity=new StringEntity(jsonObject.toString());
                    httpPost.setEntity(httpiEntity);
                    org.apache.http.HttpResponse response = httpClient.execute(httpPost);
                    return_text= HTTPResponse.readResponse(response);
...
//                        Toast.makeText(mListener.getBaseContext(), sucess, Toast.LENGTH_SHORT).show();
                        Toast.makeText(mListener.getBaseContext(), "Please Go out", Toast.LENGTH_SHORT).show();
                        JSONArray message = jsonObject.getJSONArray("message");
                        if (message!= null) {
                            ...
                            for (int i = 0; i < message.length(); i++) {
                                ...
                                String OutImage=m.getString("out_image");
                                Status=m.getString("status");
//                                prefstatus=mListener.getBaseContext().getSharedPreferences(Statusman, MODE_PRIVATE);
//                                SharedPreferences.Editor editor = prefstatus.edit();
//                                editor.putString("statm",Status);
//                                editor.commit();
                                String date=m.getString("date");
                                String intime=m.getString("in_time");
                                String outtime=m.getString("out_time");
                                ...
                                Picasso.with(mListener.getBaseContext()).load(img+InImage).into(galleryimage);
                                im = mListener.getBaseContext().getSharedPreferences("img", MODE_PRIVATE);
                                SharedPreferences.Editor editor1 = im.edit();
                                editor1.clear();
                                editor1.commit();
//                                try {
//                                    Glide.with(getContext()).load(InImage)
//                                            .thumbnail(0.5f)
//                                            .crossFade()
//                                            .diskCacheStrategy(DiskCacheStrategy.ALL)
//                                            .into(galleryimage);
...
//                            System.out.println("===message===="+sucess);
                        ...
                        if (message != null) {
                            {
                                System.out.println("===========================error");
                                Toast.makeText(mListener.getBaseContext(), message, Toast.LENGTH_SHORT).show();
                            }
                        }
                    }
                } catch (JSONException e) {
